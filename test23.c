...
void test23b(void);
void test23c(void);
void makelongnames(void);	/* Fill MaxName etc. */
char *last_index(char *string, int ch);
char *my_getcwd(char *buf, int size);
void e(int number);
void quit(void);
...
{				/* Test critical values. */
  ...
  if (getcwd(buf, strlen(cwd) + 2) != buf) e(6);	/* size just ok */
...
#else
# error "this case requires dynamic checks and is not handled"
#endif
